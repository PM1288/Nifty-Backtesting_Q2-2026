...
function pickCurrentExpiry(expiries: string[]): string {
...
export class NseOptionChainClient {
  ...
  private async fetchContractInfo(symbol: string): Promise<{ expiryDates: string[] }> {
    const ctx = await this.ensureContext();
  ...
  async fetchOptionChainV3(
    ...
    const { expiryDates } = await this.fetchContractInfo(symbol);
    const expiryRaw = pickCurrentExpiry(expiryDates);
