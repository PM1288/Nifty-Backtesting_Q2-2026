...
function DirectionTable({ rows, direction, onInspect }: { rows: ThreeMonthStrategyRow[]; direction: ThreeMonthDirection; onInspect: (value: { row: ThreeMonthStrategyRow; direction: ThreeMonthDirection }) => void }) {
  ...
  return <section className={styles.lane} data-direction={direction.toLowerCase()}>
    ...
    <div className={styles.viewport}><table><thead><tr><th>Stock</th><th className={styles.history}>M−3 OR M−2 OR M−1</th><th>M</th><th>W</th><th>D</th><th>1H</th><th>15m</th><th>Score</th></tr></thead>
...
export function ThreeMonthSelectorBoard() {
  ...
  return <section className={styles.board} data-testid="home-three-month-selector">
    <header className={styles.title}><div><strong>3MONTH REVERSAL SELECTOR</strong><small>M−3 OR M−2 OR M−1 = one point · all M/W/D/1H/15m pairs remain mandatory</small></div><Link to="/strategy/three-month">Full evidence</Link></header>
    ...
    {inspected ? <aside className={styles.drawer} role="dialog" aria-modal="true" aria-label={`${inspected.row.symbol} ${inspected.direction} 3Month arithmetic`}><button type="button" onClick={() => setInspected(null)} aria-label="Close 3Month arithmetic">×</button><h2>{inspected.row.symbol} · {inspected.direction}</h2><p>{evaluation(inspected.row, inspected.direction).qualification} · {evaluation(inspected.row, inspected.direction).scoredConditionCount}/{evaluation(inspected.row, inspected.direction).totalConditionCount} scored conditions</p><h3>M−3 OR M−2 OR M−1 · one scored group</h3><ul>{historyNewestLast(evaluation(inspected.row, inspected.direction)).map((gate) => <li key={gate.id} data-state={state(gate)}><b>{glyph(gate)} {gate.label}</b><span>{gate.left?.toFixed(2) ?? "—"} {gate.operator} {gate.right?.toFixed(2) ?? "—"}</span></li>)}</ul><h3>Mandatory M/W/D/1H/15m conditions</h3><ol>{evaluation(inspected.row, inspected.direction).gates.map((gate) => <li key={gate.id} data-state={state(gate)}><b>{glyph(gate)} {gate.label}</b><span>{gate.left?.toFixed(2) ?? "—"} {gate.operator} {gate.right?.toFixed(2) ?? "—"}</span></li>)}</ol><Link to={`/analytics/stock/${encodeURIComponent(inspected.row.symbol)}`}>Open Stock 360</Link></aside> : null}
  </section>;
}
