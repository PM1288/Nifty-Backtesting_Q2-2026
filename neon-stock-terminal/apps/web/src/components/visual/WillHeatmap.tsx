...
const WILLR_COL = 62;
const LABEL_GUTTER_TOTAL = SYMBOL_COL + WILLR_COL + 16;
...
export function WillHeatmap({ payload, selectedSymbol, onSelectSymbol }: Props) {
  ...
  const orderedRows = useMemo(
    () =>
      payload.rows
        ...
        .sort((a, b) => b.row.latestWillr - a.row.latestWillr),
  ...
  const currentSnapshot = useMemo(() => {
    ...
    orderedRows.forEach((entry, index) => {
      next.set(entry.row.symbol, {
        index,
        latestWillr: entry.row.latestWillr,
        values: entry.values
      });
    });
    return next;
  ...
  useEffect(() => {
    ...
    ctx.fillText("WILLR", SYMBOL_COL + 8, 18);
    ctx.restore();
    ...
    layout.rows.forEach(({ row, values, y }, targetIndex) => {
      ...
      const latestWillr = mix(previous?.latestWillr ?? row.latestWillr, row.latestWillr, ease);
      const currentTone = getHeatmapColor("willr", latestWillr);
      ctx.font = bodyFont;
      ctx.fillStyle = currentTone;
      ctx.fillText(row.symbol, 10, animatedY + 8);
      ctx.fillText(formatNumber(latestWillr, { minimumFractionDigits: 1, maximumFractionDigits: 1 }), SYMBOL_COL + 8, animatedY + 8);
      ...
      for (let col = 0; col < values.length; col += 1) {
        ...
        ctx.fillStyle = getHeatmapColor("willr", animatedValue);
        ctx.fillRect(
      ...
      if (previous && Math.abs(previous.latestWillr - row.latestWillr) > 0.8 && animProgress < 1) {
        ctx.strokeStyle = row.latestWillr >= previous.latestWillr ? "rgba(0,255,102,0.55)" : "rgba(255,0,51,0.55)";
        ctx.lineWidth = 1;
  ...
  return (
    <div
      ...
      ref={wrapRef}
      role="img"
      aria-label={tr("Intraday WILLR heatmap by stock and time")}
