...
| `strategy_eval.*` | NIFTY StratLab strategy assurance layer | `db/sql/020_strategy_evaluation_roe.sql` | Additive Rules-of-Engagement policies, historical event ledger, point-in-time stock/index regimes, validation/rankability, trade-path evidence, suitability slices and artifact manifests. References but does not replace `nse_app` canonical backtest facts. |
| `oiis.*` | NIFTY StratLab OIIS research layer | `db/sql/021_oiis_research.sql` | Immutable OIIS formula, replay, decision, outcome, regime-performance and artifact evidence. Raw EOD, universe, sector and index facts remain with their existing owners. |
