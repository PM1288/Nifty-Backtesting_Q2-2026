...
export function NiftyWeeklyOptionsPage() {
  ...
  return <main className={styles.page}>
    ...
    <nav className={styles.tabs} aria-label="NIFTY Options views">{VIEWS.map((item) => <button key={item} type="button" data-active={view === item} onClick={() => setParams(item === "command" ? {} : { view: item })}>{({ command: "Command Centre", weekly: "Weekly", monthly: "Monthly", chain: "Chain & Surface", paper: "Paper Book", validation: "Validation & Health" } as const)[item]}</button>)}<Link to="/strategy/trading-analytics">Trading Analytics journey</Link></nav>
