...
try {
  ...
  await page.getByRole('button',{name:'Add manual selection',exact:true}).click();
  await page.getByRole('button',{name:'Remove manual long',exact:true}).first().waitFor();
  ...
  await page.getByRole('button',{name:'Remove manual long',exact:true}).first().click();
  assert.equal(await page.getByRole('button',{name:'Remove manual long',exact:true}).count(),0); results.push('Manual removal');
