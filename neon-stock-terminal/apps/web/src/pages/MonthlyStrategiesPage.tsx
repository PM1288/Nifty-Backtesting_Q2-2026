...
export function RollingWindowStrategyPage() {
  ...
  const [failureReason, setFailureReason] = useState("ALL");
  const [stockFilters, setStockFilters] = useState<StockProfileFilters>({ universe: "ALL", capBucket: "ALL", sector: "ALL" });
  useEffect(() => { let active = true; fetchRollingWindowDashboard().then((value) => { if (active) setPayload(value); }).catch((reason) => { if (active) setError(reason instanceof Error ? reason.message : String(reason)); }); return () => { active = false; }; }, []);
  ...
  return <main className={styles.page}><header className={styles.hero}><div><span>INDEPENDENT ROLLING RESEARCH · 5 / 30 / 60 EXCHANGE SESSIONS</span><h1>Rolling Strategy</h1><p>Calendar-independent signal transitions built from trailing 60-session, 30-session, 10-session and 5-session comparisons, with next-session entry and a 30-session evidence horizon.</p></div><nav><Link to="/strategy/monthly">Monthly anchors</Link><Link className={styles.activeTab} to="/strategy/rolling-monthly">Rolling 5/30/60</Link></nav></header>
    {error ? <div className={styles.error}><b>Rolling evidence unavailable</b><span>{error}</span></div> : !payload ? <div className={styles.loading}>Calculating rolling 5/30/60 evidence…</div> : <><div className={styles.context}><strong>{rows.length} visible / {allRows.length} total</strong><label>Population<select value={population} onChange={(e) => setPopulation(e.target.value)}><option value="OPPORTUNITIES">Historical selected opportunities</option><option value="ALL">Latest all-stock review</option><option value="SELECTED">Latest newly selected</option><option value="QUALIFIED_CONTINUATION">Qualified · no new entry</option><option value="REJECTED">Latest not selected</option><option value="INCOMPLETE">Latest incomplete data</option></select></label><label>Failure reason<select value={failureReason} onChange={(e) => setFailureReason(e.target.value)}><option value="ALL">All reasons</option>{failureReasons.map((reason) => <option key={reason} value={reason}>{reasonLabel(reason)}</option>)}</select></label><span>{payload.universeRule}</span><span>{payload.refreshCadence}</span><button type="button" onClick={() => csvDownload(rows, "rolling-5-30-60-evidence.csv")}>Download filtered CSV</button></div><StockUniverseFilterBar profiles={profiles.payload?.records ?? []} filters={stockFilters} onChange={setStockFilters} count={rows.length} />
