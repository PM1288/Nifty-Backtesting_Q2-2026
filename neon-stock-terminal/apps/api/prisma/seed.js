...
function dayKeyUtc() {
    return luxon_1.DateTime.now().setZone(MARKET_TZ).startOf("day").toUTC().toJSDate();
}
function marketTsUtc(hour, minute) {
...
async function main() {
    ...
    for (const stock of allStocks) {
        const base = stock.assetType === client_1.AssetType.INDEX ? rand(21000, 24000) : rand(50, 3500);
