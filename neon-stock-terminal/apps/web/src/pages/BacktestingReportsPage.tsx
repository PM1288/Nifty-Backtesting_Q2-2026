...
export function BacktestingReportsPage() {
  ...
  return <main className={styles.page} data-testid="backtesting-reports">
    ...
    {query.isLoading ? <section className={styles.state}>Loading report catalogue…</section> : query.isError || !report ? <section className={styles.state} role="alert">The generated report is not currently mounted. No placeholder results are shown.</section> : <>
      ...
      <section className={styles.notes}><h2>Report contents</h2><p>The first PDF page combines the exact Bull/Bear conditions, limitations, colour legend and aggregate summary. Blue upward markers identify dated Bull qualifications; yellow downward markers identify dated Bear qualifications. Markers appear only on the Daily chart at their exact dates. Weekly and Monthly candles provide context and are not assigned both directions.</p><p>Bull and Bear are mutually exclusive for a stock on the same date. Because the current month is still forming, a stock can cross its monthly reference opens and qualify in opposite directions on different dates in that month. The PDF does not duplicate thousands of trade rows. Download the CSV for every signal, entry, exact gate result, reference value and return. Historical 1H/15m confirmation is not claimed because retained minute data does not cover the full evaluation year.</p></section>
    </>}
  </main>;
}
