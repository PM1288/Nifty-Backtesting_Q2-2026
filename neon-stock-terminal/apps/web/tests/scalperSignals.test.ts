...
test("paired body80 CALL needs two red below-EMA candles on NIFTY and exact CE", () => {
  const panes = [
    { identity: identity("NIFTY 50"), bars: [bar(t[0], 100, 98, 101), bar(t[1], 99, 97, 100), bar(t[2], 99, 109, 101), bar(t[3], 104, 106, 103)] },
    { identity: identity("NIFTY08SEP23600CE", "NFO", "ce"), bars: [bar(t[0], 24, 22, 25), bar(t[1], 23, 21, 24), bar(t[2], 22, 32, 24), bar(t[3], 30, 31, 29)] },
...
test("rejects missing option confirmation, one precursor, non-consecutive and 79.99 percent", () => {
  ...
  const oneRed = [bar(t[0], 22, 23, 25), bar(t[1], 23, 21, 24), bar(t[2], 22, 32, 24), bar(t[3], 30, 31, 29)];
  assert.equal(scalperPairedBody80Signals([{ identity: identity("NIFTY 50"), bars: nifty }, { identity: identity("XCE", "NFO"), bars: oneRed }], 5).length, 0);
