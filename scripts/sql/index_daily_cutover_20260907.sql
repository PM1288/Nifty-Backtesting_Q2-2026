...
SELECT a.trade_date,a.symbol_token,a.close::numeric(18,6) close_px
...
SELECT (b.ts AT TIME ZONE 'Asia/Kolkata')::date,b.symbol_token,
 (array_agg(b.close ORDER BY b.ts DESC))[1]::numeric(18,6)
...
minute_fallback AS (SELECT d.trade_date,im.index_code,d.close_px FROM integration.v_archived_index_close d JOIN index_map im USING(symbol_token))
SELECT trade_date,index_code,close_px FROM market_activity WHERE index_code IS NOT NULL
UNION ALL
SELECT mf.trade_date,mf.index_code,mf.close_px FROM minute_fallback mf WHERE NOT EXISTS(
