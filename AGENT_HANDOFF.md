...
## 2026-08-26 — Concise paper speech and ticker-free header

- Browser speech now defaults ON when no preference exists while continuing to respect an explicit user mute. Entry speech contains only the canonical company/stock name, entry price and governed target price; target speech contains only `Target hit` and the company/stock name. The client derives this concise phrase from structured fields and does not read verbose notification bodies or strategy names.
- `/v1/paper/notifications` now returns `stockName`, `entryPrice` and `targetPrice`. Company names are resolved from the durable event payload or `public.instrument_profiles`; absent data remains absent rather than inferred. Existing popup text, event history and deep links remain unchanged.
...
# 2026-09-07 — Scalper 5m/day view and preview resistance

Scoped branch `ui/scalper-5m-levels-20260907`; see `docs/trading-analytics/SCALPER_5M_LEVELS_20260907.md`. Defaults: five-minute bars, latest retained IST session. Green/red candles, NIFTY-only 50-point grid, source-backed monthly resistance and configurable daily/weekly lookbacks. Unapproved resistance remains research-only. API 165/165 and web 77/77 tests, typechecks/builds and canonical gate pass before rollout. No trading execution or database changes.

Final deployed acceptance: application `b2077c2`, dashboard image `sha256:58033a33bd2f248854906a2329e9b1c31708ad8499018f5e9d11c27657b36ff3`; authenticated desktop/mobile browser checks 22/22, main-content axe zero violations at both widths. Loaded screenshots, raw levels and results: `output/playwright/scalper-5m-20260907/`. Monthly uses 12 completed months; daily/weekly remain explicit user choices (20/12 in fixtures only). NIFTY ticks anchor on multiples of 50 and enabled R lines fit the axis. Last harness-only change waits for loaded charts before screenshots; no runtime change after deployed application commit.
...
# 2026-09-08 — Scalper A-open / B-close measurement semantics

- The browser-only exact pair measurement now takes entry from the selected A candle's completed `open` and exit from the selected B candle's completed `close` for underlying, CE and PE. Markers/table labels make that explicit; temporal matching remains strict and missing bars remain unavailable.
- Deployed commits: `89433a6`, `8adff03`. Authenticated production regression passed 22/22: exact source open-to-close P&L, quantity scaling, chart selection, zoom persistence, reload clearing, desktop/mobile axe, overflow and browser-error checks. Evidence: `output/playwright/scalper-measurement/results.json` and `measurement-desktop.png`.
# 2026-09-08 — MANEESH aligned candlestick terminal V3

- Reviewed the complete `/home/novius2/NIFTY50/UI` Markdown/JSON/image/ZIP source pack and implemented it additively in the canonical repository. Detailed mapping and limitations: `docs/trading-analytics/ALIGNED_TERMINAL_V3_20260908.md`.
- Pushed master commit `650a589`. The Scalper defaults to a lazy-loaded Lightweight Charts 5.2.1 seven-pane terminal: NIFTY, exact CE, exact PE, outstanding OI, signed interval ΔOI, RSI 14 and MACD 12/26/9. The full previous ECharts surface remains available with `renderer=classic`.
- Added a synchronized cursor, compact right evidence rail, provider-native strike/OI profile, strict selected-session level display, indicative max-pain references, green/red candles, orange EMA9, CE-blue/PE-yellow identity, and browser-only A-open/B-close measurement rectangles.
- Added research-only `NIFTY_EMA9_BODY70_NEXT_OPEN_V3` closed-bar references. Exact next scheduled bar is required; missing bars are not skipped; no paper/live eligibility or order path was added.
...
- Acceptance ZIP:
  ...
  SHA-256 `495ffc6fdc0df39f72ada2c0e69734186dfac324f6dde111cd5f7a003dae7a04`.
...
Released from master commit `280596c` on 2026-09-23. Focused positioning tests passed 4/4; the prior full web/API/typecheck/build/canonical-gate run passed before release. Authenticated production Playwright passed 23/23 after the rolling deployment settled, including geometry, rate-view identification, in-place refresh, zero hover network, no chart remount and no page errors. Evidence: `/home/novius2/NIFTY50/evidence/scalper-v2-oi-rate-by-strike-20260923/production-final/`.
## 2026-09-23 — Scalper V2 connected chart canvas
