...
export function BacktestingReportsPage() {
  ...
  return <main className={styles.page} data-testid="backtesting-reports">
    ...
    {query.isLoading ? <section className={styles.state}>Loading report catalogue…</section> : query.isError || !report ? <section className={styles.state} role="alert">The generated report is not currently mounted. No placeholder results are shown.</section> : <>
      ...
      <section className={styles.notes}><h2>Report contents</h2><p>The first PDF page combines the exact Bull/Bear conditions, limitations, colour legend and aggregate summary. Blue upward markers identify Bull qualifications; yellow downward markers identify Bear qualifications. Daily triangles retain exact dates. Weekly and Monthly triangles aggregate those events into the containing candle, with ×N showing the number of dated qualifications rather than assigning a direction to the whole candle.</p><p>The Daily chart includes dashed month boundaries, dotted week boundaries and distinct month/week open and retrospective final-close levels. Bull and Bear remain mutually exclusive for a stock on the same date. The PDF does not duplicate thousands of trade rows. Download the CSV for every signal, entry, gate result, Monthly/Weekly/Daily OHLC reference and return. Historical 1H/15m confirmation is not claimed because retained minute data does not cover the full evaluation year.</p></section>
    </>}
  </main>;
}
