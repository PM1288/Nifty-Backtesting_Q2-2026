...
export function OissV1Page() {
  ...
  return <section aria-label="OISS decision workspace" className={styles.page}>
    ...
    {lens === "sectors" ? <SectorLens rows={data.sectors}/> : lens === "changes" ? <ChangesLens rows={data.changes}/> : lens === "backtest" ? <BacktestLens data={data}/> : lens === "audit" ? <AuditLens data={data}/> : lens === "open-positions" ? <OpenPositions rows={data.paper}/> : lens === "market" || lens === "risk" ? <ContextLens run={run} lens={lens}/> :
      <section className={styles.workspace}>
        <section aria-label="Stock radar" className={styles.radar}><h2>{lens === "rejected" ? "Rejected opportunities" : "Stock radar"}</h2><div className={styles.radarRows}>{data.radar.filter((row)=>lens!=="rejected" || ['NO TRADE','NO CHASE','DATA INSUFFICIENT'].includes(row.canonical_status)).map((row)=><button key={row.candidate_id} className={`${selected?.candidate_id===row.candidate_id?styles.selected:""}`} onClick={()=>pick(row.symbol)}><span className={styles.stock}><i>{row.symbol.slice(0,2)}</i><span><b>{row.symbol}</b><small>{row.company_name || row.symbol}</small></span></span><span>{row.direction}</span><b>{n(row.tqs)}</b><em className={statusClass(row.canonical_status)}>{row.canonical_status}</em></button>)}</div></section>
...
function CandidateInspector({row}:{row:any}) { if(!row)return <section aria-label="Execution inspector"/>; return <section aria-label="Execution inspector" className={styles.inspector}><h2>Execution inspector</h2><dl><dt>Entry zone</dt><dd>{n(row.entry_plan?.entry_zone_low)}–{n(row.entry_plan?.entry_zone_high)}</dd><dt>Stop</dt><dd>{n(row.entry_plan?.stop)}</dd><dt>Target 1 / 2</dt><dd>{n(row.entry_plan?.target_1)} / {n(row.entry_plan?.target_2)}</dd><dt>Lot size</dt><dd>{row.lot_size ?? '—'}</dd><dt>Final lots</dt><dd>{row.position_sizing?.final_lots ?? '—'}</dd><dt>Planned loss</dt><dd>₹{n(row.position_sizing?.maximum_planned_loss)}</dd></dl><Link to={`/analytics/stock/${encodeURIComponent(row.symbol)}?strategy=OISS_V1_202608&source=oiss`}>Open Stock 360</Link><Link to={`/paper-trading?strategy=OISS_V1_202608&symbol=${encodeURIComponent(row.symbol)}`}>Open Paper evidence</Link><JsonBlock title="Audit snapshot" value={row.feature_snapshot}/></section> }
