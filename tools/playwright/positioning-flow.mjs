...
try {
  ...
  await dashboard.getByRole("button", { name: "Likely Levels", exact: true }).click();
  ...
  check("LIKELY-LEVELS", await levels.locator("tbody tr").count() > 0 || levelText.includes("No probable zone"), "ranked zones or truthful insufficient-evidence state visible");
  ...
  await page.screenshot({ path: path.join(output, "positioning-flow-likely-levels-1920x1080.png"), fullPage: true });
