  ...
  return <section className={css.page} data-testid="scalper-v2">
    ...
    <section className={css.analytics}><header className={css.analyticsHeader}><h2>Option analytics · latest retained snapshot</h2><span>{inspectionMode !== "latest" ? "Historical chain unavailable at inspected time · " : ""}PCR {pcr == null ? "unavailable" : pcr.toFixed(2)} · Max pain {maxPain.candidates.join(", ") || "unavailable"}</span></header>
      <div className={css.analyticsGrid}>
        <article className={css.analyticCard}><h3>OI by strike</h3><p>Current provider-native OI; zero-based · {spot == null || nearestSpotStrike == null ? "spot guide unavailable" : `spot ${number(spot)} · nearest strike ${nearestSpotStrike.toLocaleString("en-IN")}`}</p><Suspense fallback={<p>Loading chart…</p>}><Chart className={css.analyticChart} ariaLabel="OI by strike" axisExtentPolicy="native" option={analyticOptions[0]} activeCategoryIndex={hoveredStrikeIndex} onCategoryHover={(index) => setHoveredStrike(index == null ? null : strikeRows[index] ?? null)} /></Suspense></article>
