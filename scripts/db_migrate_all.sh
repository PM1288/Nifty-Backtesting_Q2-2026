...
log "additional: Home MWHD 5-minute qualification WhatsApp alert outbox"
run_compose exec -T postgres psql -U "${POSTGRES_USER}" -d "${POSTGRES_DB}" -v ON_ERROR_STOP=1 < "${ROOT_DIR}/db/sql/063_home_mw5_qualification_alert_outbox.sql"
