...
export async function loadSmartApiNifty(
  ...
  const contracts =
    ...
    LEFT JOIN LATERAL (SELECT p.oi,p.ts,p.exch_feed_time FROM public.quote_snapshots p WHERE p.exchange=i.exchange AND p.symbol_token=i.symbol_token AND p.ts BETWEEN $1::timestamptz-interval '1 day' AND $1::timestamptz AND (p.exch_feed_time AT TIME ZONE 'Asia/Kolkata')::date=($1::timestamptz AT TIME ZONE 'Asia/Kolkata')::date AND p.exch_feed_time<=$1::timestamptz AND p.oi IS NOT NULL ORDER BY p.exch_feed_time,p.ts LIMIT 1) session_first ON true
    LEFT JOIN LATERAL (SELECT p.oi,p.ts,p.exch_feed_time FROM public.quote_snapshots p WHERE p.exchange=i.exchange AND p.symbol_token=i.symbol_token AND p.ts BETWEEN previous_day.market_open_ts-interval '1 day' AND $1::timestamptz AND p.exch_feed_time BETWEEN previous_day.market_open_ts AND previous_day.market_close_ts AND p.oi IS NOT NULL ORDER BY p.exch_feed_time DESC,p.ts DESC LIMIT 1) prior_session ON true
