...
import { matchesStockProfile, useProfileIndex, type StockProfileFilters } from "../lib/stockProfiles";
...
type EvidenceRow = {
  id: string;
  entryMethod: "EXPIRY" | "MONTHLY_CLOSURE" | "FIRST_SESSION" | "ROLLING_5_30_60";
  symbol: string;
  period: string;
  signalDate: string;
  entryDate: string;
  ...
  status: string;
  selectionStatus: "SELECTED" | "REJECTED" | "INCOMPLETE" | "QUALIFIED_CONTINUATION";
...
const text = (value: unknown) => value == null ? "" : String(value).slice(0, 10);
...
const truth = (value: unknown): boolean | null => value == null ? null : value === true || value === "true";
const rejectionReasons = (row: EvidenceRow): string[] => {
  const values = Array.isArray(row.raw.rejection_reasons) && row.raw.rejection_reasons.length
    ? row.raw.rejection_reasons
    : Array.isArray(row.raw.failed_condition_codes) ? row.raw.failed_condition_codes : [];
  return values.map(String);
};
const reasonLabel = (value: string) => value.replaceAll("_", " ").toLowerCase().replace(/^./, (letter) => letter.toUpperCase());
...
function normalized(
  ...
  const fromExpiry = expiry.map((row): EvidenceRow => {
    ...
    return {
      id: String(row.candidate_id), entryMethod: "EXPIRY", symbol: String(row.symbol),
      period: text(row.expiry_month), signalDate: text(row.expiry_signal_date ?? row.signal_date),
      entryDate: text(row.expiry_entry_date ?? row.entry_date), entryPrice: entry, endPrice: end,
      endReturn, maxProfit, maxDrawdown, pnl10000: entry != null && end != null ? quantity * (end - entry) : null,
      ...
      status: String(row.expiry_evaluation_status ?? "DEVELOPING"), selectionStatus: "SELECTED", gapThreshold: null, gapPct: null,
      ema9: number(scanner.m1_monthly_ema9), closeAboveEma9: truth(scanner.m1_close_above_monthly_ema9),
      candleAboveEma9Pct: number(scanner.m1_candle_above_monthly_ema9_pct),
      hit1: (maxProfit ?? -Infinity) >= 1, hit3: (maxProfit ?? -Infinity) >= 3, hit5: (maxProfit ?? -Infinity) >= 5, raw: row,
    };
  });
  const fromClosure = closure.map((row): EvidenceRow => {
    const entry = number(row.entry_price); const end = number(row.path_end_price); const quantity = entry ? Math.floor(10000 / entry) : 0;
    const maxProfit = number(row.max_profit_pct); const maxDrawdown = number(row.max_drawdown_pct);
    return {
      id: String(row.candidate_id), entryMethod: "MONTHLY_CLOSURE", symbol: String(row.symbol), period: text(row.evaluation_month),
      signalDate: text(row.signal_date), entryDate: text(row.entry_date), entryPrice: entry, endPrice: end,
      endReturn: number(row.end_return_pct), maxProfit, maxDrawdown,
      pnl10000: entry != null && end != null ? quantity * (end - entry) : null,
      maxProfit10000: entry != null && maxProfit != null ? quantity * entry * maxProfit / 100 : null,
      maxDrawdown10000: entry != null && maxDrawdown != null ? quantity * entry * maxDrawdown / 100 : null,
      status: String(row.evaluation_status), selectionStatus: "SELECTED", gapThreshold: null, gapPct: null, ema9: number(row.monthly_ema9),
      closeAboveEma9: truth(row.monthly_close_above_ema9), candleAboveEma9Pct: number(row.monthly_candle_above_ema9_pct),
      hit1: (maxProfit ?? -Infinity) >= 1, hit3: (maxProfit ?? -Infinity) >= 3, hit5: (maxProfit ?? -Infinity) >= 5, raw: row,
    };
  });
  const fromFirst = firstSession.map((row): EvidenceRow => {
    const maxProfit = number(row.max_profit_pct); const maxDrawdown = number(row.max_drawdown_pct);
    return {
      id: String(row.candidate_id), entryMethod: "FIRST_SESSION", symbol: String(row.symbol), period: text(row.evaluation_month),
      signalDate: text(row.first_session_date), entryDate: text(row.entry_date), entryPrice: number(row.entry_price), endPrice: number(row.path_end_price),
      endReturn: number(row.end_return_pct), maxProfit, maxDrawdown, pnl10000: number(row.end_pnl_10000),
      maxProfit10000: number(row.max_profit_10000), maxDrawdown10000: number(row.max_drawdown_10000),
      status: String(row.entry_status === "ENTERED" ? row.evaluation_status : row.entry_status), selectionStatus: row.entry_status === "ENTERED" ? "SELECTED" : "REJECTED",
      gapThreshold: number(row.gap_threshold_pct), gapPct: number(row.opening_gap_pct), ema9: number(row.monthly_ema9),
      closeAboveEma9: truth(row.monthly_close_above_ema9), candleAboveEma9Pct: number(row.monthly_candle_above_ema9_pct),
      hit1: (maxProfit ?? -Infinity) >= 1, hit3: (maxProfit ?? -Infinity) >= 3, hit5: (maxProfit ?? -Infinity) >= 5, raw: row,
    };
  });
  const rejectedClosure = closureEvaluations.filter((row) => row.selection_status !== "SELECTED").map((row): EvidenceRow => ({
    id: String(row.evaluation_id), entryMethod: "MONTHLY_CLOSURE", symbol: String(row.symbol), period: text(row.evaluation_month),
    signalDate: text(row.signal_date), entryDate: "", entryPrice: null, endPrice: null, endReturn: null,
    maxProfit: null, maxDrawdown: null, pnl10000: null, maxProfit10000: null, maxDrawdown10000: null,
    status: String(row.selection_status), selectionStatus: String(row.selection_status) as EvidenceRow["selectionStatus"],
    gapThreshold: null, gapPct: null, ema9: null, closeAboveEma9: null, candleAboveEma9Pct: null,
    hit1: false, hit3: false, hit5: false, raw: { ...row, conditions: row.conditions ?? [], rejection_reasons: row.rejection_reasons ?? [] },
  }));
...
function csvDownload(rows: EvidenceRow[], filename: string) {
  const columns: Array<[string, (row: EvidenceRow) => unknown]> = [
    ["entry_method", (r) => r.entryMethod], ["symbol", (r) => r.symbol], ["period", (r) => r.period],
    ["signal_date", (r) => r.signalDate], ["entry_date", (r) => r.entryDate], ["entry_price", (r) => r.entryPrice],
    ["status", (r) => r.status], ["end_return_pct", (r) => r.endReturn], ["max_profit_pct", (r) => r.maxProfit],
    ["max_drawdown_pct", (r) => r.maxDrawdown], ["pnl_10000", (r) => r.pnl10000], ["monthly_ema9", (r) => r.ema9],
    ["close_above_ema9", (r) => r.closeAboveEma9], ["candle_above_ema9_pct", (r) => r.candleAboveEma9Pct],
  ];
  const quote = (value: unknown) => `"${String(value ?? "").replaceAll('"', '""')}"`;
  const body = [columns.map(([label]) => label).join(","), ...rows.map((row) => columns.map(([, getter]) => quote(getter(row))).join(","))].join("\n");
  const url = URL.createObjectURL(new Blob([`\uFEFF${body}`], { type: "text/csv;charset=utf-8" }));
  const anchor = document.createElement("a"); anchor.href = url; anchor.download = filename; anchor.click(); URL.revokeObjectURL(url);
}

function Kpi({ label, value, detail }: { label: string; value: string; detail: string }) {
  return <article className={styles.kpi}><span>{label}</span><strong>{value}</strong><small>{detail}</small></article>;
}
...
function Inspector({ row, onClose }: { row: EvidenceRow; onClose: () => void }) {
  const conditions = Array.isArray(row.raw.conditions) ? row.raw.conditions : Object.entries(row.raw.conditions ?? {}).map(([label, pass]) => ({ label, pass }));
  return <aside className={styles.inspector} aria-label={`${row.symbol} strategy evidence`}><header><div><span>{row.entryMethod.replaceAll("_", " ")}</span><h2>{row.symbol} · {row.period.slice(0, 7)}</h2></div><button type="button" onClick={onClose} aria-label="Close inspector">×</button></header>
    <div className={styles.inspectorGrid}><Kpi label="Entry" value={price(row.entryPrice)} detail={row.entryDate || "Not entered"} /><Kpi label="End / as-of" value={pct(row.endReturn)} detail={row.status} /><Kpi label="Maximum profit" value={pct(row.maxProfit)} detail={money(row.maxProfit10000)} /><Kpi label="Maximum drawdown" value={pct(row.maxDrawdown)} detail={money(row.maxDrawdown10000)} /></div>
    <section><h3>Entry conditions</h3>{conditions.length ? <ul className={styles.conditions}>{conditions.map((condition: any, index: number) => <li key={String(condition.code ?? condition.label ?? index)} data-pass={condition.pass === true}><span>{condition.pass === true ? "✓" : condition.informational ? "i" : "×"}</span><div><b>{String(condition.label ?? condition.code ?? "Condition").replaceAll("_", " ")}</b>{condition.left != null || condition.right != null ? <small>{String(condition.left ?? "—")} {condition.operator ?? ""} {String(condition.right ?? "—")}</small> : null}</div></li>)}</ul> : <p>No structured condition trace was persisted for this row.</p>}</section>
    {row.entryMethod === "ROLLING_5_30_60" ? <section><h3>Rolling-window context</h3><p>Older 60-session open {price(number(row.raw.older_block_open))}; older block close {price(number(row.raw.older_block_close))}; recent 30-session open {price(number(row.raw.recent_block_open))}; 10-session and 5-session opens {price(number(row.raw.prior_week_open))} / {price(number(row.raw.current_week_open))}.</p></section> : <section><h3>EMA9 context — informational</h3><p>Monthly EMA9 is never used as a silent entry gate. Close: {row.closeAboveEma9 == null ? "not available" : row.closeAboveEma9 ? "above EMA9" : "below EMA9"}; bullish candle body above EMA9: {pct(row.candleAboveEma9Pct)}.</p></section>}
    {rejectionReasons(row).length ? <section><h3>Why it was not selected</h3><ul className={styles.reasonList}>{rejectionReasons(row).map((reason) => <li key={reason}>{reasonLabel(reason)}</li>)}</ul></section> : null}
    <section><h3>Research economics</h3><dl><div><dt>₹10k end P&L</dt><dd>{money(row.pnl10000)}</dd></div><div><dt>₹10k maximum opportunity</dt><dd>{money(row.maxProfit10000)}</dd></div><div><dt>₹10k maximum pain</dt><dd>{money(row.maxDrawdown10000)}</dd></div></dl></section>
    <footer><Link to={`/analytics/stock/${encodeURIComponent(row.symbol)}?strategy=${row.entryMethod.toLowerCase()}&asOf=${row.signalDate}`}>Open Stock 360</Link><button type="button" onClick={() => csvDownload([row], `${row.symbol}-${row.entryMethod}.csv`)}>Download row</button></footer>
  </aside>;
}
...
export function RollingWindowStrategyPage() {
  const profiles = useProfileIndex(); const [payload, setPayload] = useState<any>(null); const [error, setError] = useState(""); const [selected, setSelected] = useState<EvidenceRow | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const compact = await fetchRollingWindowDashboard(undefined, undefined, 250);
  ...
  const opportunityRows = useMemo<EvidenceRow[]>(() => (payload?.rows ?? []).map((raw: any): EvidenceRow => ({ id: String(raw.candidate_id), entryMethod: "ROLLING_5_30_60", symbol: String(raw.symbol), period: text(raw.signal_date), signalDate: text(raw.signal_date), entryDate: text(raw.entry_date), entryPrice: number(raw.entry_price), endPrice: number(raw.path_end_price), endReturn: number(raw.end_return_pct), maxProfit: number(raw.max_profit_pct), maxDrawdown: number(raw.max_drawdown_pct), pnl10000: number(raw.pnl_10000), maxProfit10000: number(raw.max_profit_10000), maxDrawdown10000: number(raw.max_drawdown_10000), status: String(raw.evaluation_status), selectionStatus: "SELECTED", gapThreshold: null, gapPct: null, ema9: null, closeAboveEma9: null, candleAboveEma9Pct: null, hit1: truth(raw.hit_1_pct) === true, hit3: truth(raw.hit_3_pct) === true, hit5: truth(raw.hit_5_pct) === true, raw: { ...raw, conditions: raw.conditions } })), [payload]);
  const evaluationRows = useMemo<EvidenceRow[]>(() => (payload?.evaluations ?? []).map((raw: any): EvidenceRow => ({ id: String(raw.evaluation_id), entryMethod: "ROLLING_5_30_60", symbol: String(raw.symbol), period: text(raw.signal_date), signalDate: text(raw.signal_date), entryDate: "", entryPrice: null, endPrice: null, endReturn: null, maxProfit: null, maxDrawdown: null, pnl10000: null, maxProfit10000: null, maxDrawdown10000: null, status: String(raw.selection_status), selectionStatus: String(raw.selection_status) as EvidenceRow["selectionStatus"], gapThreshold: null, gapPct: null, ema9: null, closeAboveEma9: null, candleAboveEma9Pct: null, hit1: false, hit3: false, hit5: false, raw: { ...raw, ...(raw.factor_values ?? {}), conditions: raw.conditions ?? [], rejection_reasons: raw.rejection_reasons ?? [] } })), [payload]);
  const allRows = population === "OPPORTUNITIES" ? opportunityRows : evaluationRows.filter((row) => population === "ALL" || row.selectionStatus === population);
  const failureReasons = useMemo(() => Array.from(new Set(evaluationRows.flatMap(rejectionReasons))).sort(), [evaluationRows]);
  const rows = useMemo(() => allRows.filter((row) => (failureReason === "ALL" || rejectionReasons(row).includes(failureReason)) && matchesStockProfile(profiles.bySymbol.get(row.symbol), stockFilters)), [allRows, failureReason, profiles.bySymbol, stockFilters]);
  const summary = payload?.summary ?? {}; const visibleProfiles = rows.map((row) => profiles.bySymbol.get(row.symbol)).filter(Boolean) as NonNullable<ReturnType<typeof profiles.bySymbol.get>>[];
  return <main className={styles.page}><header className={styles.hero}><div><span>INDEPENDENT ROLLING RESEARCH · 5 / 30 / 60 EXCHANGE SESSIONS</span><h1>Rolling Strategy</h1><p>Calendar-independent signal transitions built from trailing 60-session, 30-session, 10-session and 5-session comparisons, with next-session entry and a 30-session evidence horizon.</p></div><nav><Link to="/strategy/monthly">Monthly anchors</Link><Link className={styles.activeTab} to="/strategy/rolling-monthly">Rolling 5/30/60</Link></nav></header>
    {error ? <div className={styles.error}><b>{payload ? "Complete rolling history could not be refreshed" : "Rolling evidence unavailable"}</b><span>{error}</span></div> : null}
    {!payload ? <div className={styles.loading}>Calculating rolling 5/30/60 evidence…</div> : <>{historyLoading && payload.historyLimited ? <div className={styles.loading}>Latest 250 opportunities ready · loading the complete historical ledger in the background…</div> : null}<div className={styles.context}><strong>{rows.length} visible / {allRows.length} {payload.historyLimited ? "currently loaded" : "total"}</strong><label>Population<select value={population} onChange={(e) => setPopulation(e.target.value)}><option value="OPPORTUNITIES">Historical selected opportunities</option><option value="ALL">Latest all-stock review</option><option value="SELECTED">Latest newly selected</option><option value="QUALIFIED_CONTINUATION">Qualified · no new entry</option><option value="REJECTED">Latest not selected</option><option value="INCOMPLETE">Latest incomplete data</option></select></label><label>Failure reason<select value={failureReason} onChange={(e) => setFailureReason(e.target.value)}><option value="ALL">All reasons</option>{failureReasons.map((reason) => <option key={reason} value={reason}>{reasonLabel(reason)}</option>)}</select></label><span>{payload.universeRule}</span><span>{payload.refreshCadence}</span><button type="button" onClick={() => csvDownload(rows, "rolling-5-30-60-evidence.csv")} disabled={historyLoading && population === "OPPORTUNITIES"}>Download filtered CSV</button></div><StockUniverseFilterBar profiles={profiles.payload?.records ?? []} filters={stockFilters} onChange={setStockFilters} count={rows.length} />
      <section className={styles.kpis}><Kpi label="Opportunities" value={String(summary.opportunities ?? allRows.length)} detail={`${summary.matured ?? 0} mature · ${summary.developing ?? 0} developing`} /><Kpi label="Positive at D+30 / as-of" value={summary.winRatePct == null ? "—" : `${Number(summary.winRatePct).toFixed(2)}%`} detail={`${summary.winners ?? 0} winners`} /><Kpi label="₹10k each · end P&L" value={money(number(summary.pnl10000))} detail="Gross, all evaluable transitions" /><Kpi label="Max reward / pain" value={`${money(number(summary.maxProfit10000))} / ${money(number(summary.maxDrawdown10000))}`} detail="Observed path extrema" /></section>
      <StockDistribution profiles={visibleProfiles} /><StrategyTable rows={rows} title="Rolling 5/30/60 signal transitions" profiles={profiles.bySymbol} onSelect={setSelected} /></>}{selected ? <Inspector row={selected} onClose={() => setSelected(null)} /> : null}</main>;
}
...
export function RollingMonthlyLegacyRouter() {
  ...
  if (["expiry", "absolute", "absolute-first-session"].includes(view ?? "")) {
    const method = view === "expiry" ? "EXPIRY" : view === "absolute-first-session" ? "FIRST_SESSION" : "MONTHLY_CLOSURE";
