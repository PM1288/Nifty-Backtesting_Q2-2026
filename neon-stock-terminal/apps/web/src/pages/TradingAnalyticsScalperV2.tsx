...
export function TradingAnalyticsScalperV2({ symbol, label, asOf, expiry, strikes, spot, legs, metricLegs = [], state, errors = [], referenceLevels, layout = "v2" }: {
  ...
  const [oiDisplayMode, setOiDisplayMode] = useState<ScalperV2OiDisplayMode>(() => (
    typeof window !== "undefined" && window.localStorage.getItem("n50.scalper-v2.oi-display") === "underlying_units"
      ? "underlying_units"
      : "contracts"
  ));
  ...
  useEffect(() => {
    if (typeof window !== "undefined") window.localStorage.setItem("n50.scalper-v2.oi-display", oiDisplayMode);
  }, [oiDisplayMode]);
