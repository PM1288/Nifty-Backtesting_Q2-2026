...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--database-url',default=os.getenv('DATABASE_URL') or os.getenv('TRADING_DATABASE_URL')); p.add_argument('--start',default='2000-01-01'); p.add_argument('--end',default=(date.today()+timedelta(days=1)).isoformat()); p.add_argument('--limit',type=int); p.add_argument('--csv-input',type=Path); p.add_argument('--output-dir',type=Path,default=ROOT/'platform/nifty_stratlab/outputs/nifty500_yfinance_regime'); a=p.parse_args();
    ...
    with pd.ExcelWriter(a.output_dir/'stock_daily_regime.xlsx',engine='openpyxl') as writer:
