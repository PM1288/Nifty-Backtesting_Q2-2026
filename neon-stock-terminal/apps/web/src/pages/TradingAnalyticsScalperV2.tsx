...
import { scaleScalperV2Oi, scaleScalperV2OiTimePoints, scaleScalperV2ProfileRows, scalperV2OiLotContext, scalperV2OiMultiplier, type ScalperV2OiDisplayMode } from "../lib/scalperV2OiDisplay";
...
export function TradingAnalyticsScalperV2({ symbol, label, asOf, expiry, strikes, spot, legs, metricLegs = [], state, errors = [], referenceLevels, layout = "v2" }: {
  ...
  const [oiDisplayMode, setOiDisplayMode] = useState<ScalperV2OiDisplayMode>(() => (
    typeof window !== "undefined" && window.localStorage.getItem("n50.scalper-v2.oi-display-v2") === "contracts"
      ? "contracts"
      : "underlying_units"
  ));
  ...
  useEffect(() => { setFitRequest((value) => value + 1); setVerticalView("session"); setYLocked(false); setLockedTime(null); setHoverCrosshair(null); }, [symbol, expiry, selectedCeStrike, selectedPeStrike, interval, selectedDayParam]);
  ...
  const oiLotContext = useMemo(() => scalperV2OiLotContext(rankSource), [rankSource]);
  const effectiveOiDisplayMode: ScalperV2OiDisplayMode = oiDisplayMode === "underlying_units" && oiLotContext.available
    ? "underlying_units"
    : "contracts";
  const oiDisplayMultiplier = scalperV2OiMultiplier(effectiveOiDisplayMode, oiLotContext.lotSize);
  const oiDisplayLabel = effectiveOiDisplayMode === "underlying_units"
    ? `Underlying units · OI × ${oiLotContext.lotSize}`
    : "OI contracts";
  const oiDisplayShortLabel = effectiveOiDisplayMode === "underlying_units" ? `units ×${oiLotContext.lotSize}` : "contracts";
  const displayOi = useCallback((value: unknown) => {
    const parsed = numeric(value);
    return parsed == null ? null : scaleScalperV2Oi(parsed, oiDisplayMultiplier);
  }, [oiDisplayMultiplier]);
  useEffect(() => {
    if (typeof window !== "undefined") window.localStorage.setItem("n50.scalper-v2.oi-display-v2", oiDisplayMode);
  }, [oiDisplayMode]);
  const leaders = useMemo(() => rankCurrentOi(rankSource), [rankSource]);
  const opposingOiGuides = useMemo(() => scalperV2OpposingOiGuides(leaders), [leaders]);
  const displayLeaders = useMemo(() => leaders.map((leader) => ({
    ...leader,
    currentOi: scaleScalperV2Oi(leader.currentOi, oiDisplayMultiplier)!,
    changeOi: scaleScalperV2Oi(leader.changeOi, oiDisplayMultiplier),
  })), [leaders, oiDisplayMultiplier]);
  ...
  const rawProfileRows = profileModel.rows;
  const profileRows = useMemo(() => scaleScalperV2ProfileRows(rawProfileRows, oiDisplayMultiplier), [oiDisplayMultiplier, rawProfileRows]);
  ...
  const snapshotDays = [...new Set(rawProfileRows.map((row) => istDay(row.currentAt)).filter(Boolean))].sort();
  ...
  const deltaState = useMemo(() => oiComparisonState(
    rawProfileRows.map((row) => row.currentOi),
    rawProfileRows.map((row) => row.state === "comparable" ? row.baselineOi : null),
  ), [rawProfileRows]);
  ...
  const displayCumulativeOiPoints = useMemo(
    () => scaleScalperV2OiTimePoints(cumulativeOiPoints, oiDisplayMultiplier),
    [cumulativeOiPoints, oiDisplayMultiplier],
  );
  ...
  const cumulativeLatest = displayCumulativeOiPoints.at(-1) ?? null;
  ...
  const cumulativeOiOption = useMemo<EChartsOption>(() => ({
    ...
    yAxis: {
      type: "value",
      min: 0,
        name: `Total OI · ${oiDisplayShortLabel}`,
    ...
    series: [
      {
        name: "CE Total OI",
        type: "line",
        data: displayCumulativeOiPoints.map((point) => [Date.parse(point.capturedAt), point.ceOi]),
        connectNulls: false,
        showSymbol: displayCumulativeOiPoints.length <= 1,
        symbolSize: 7,
      ...
      },
      {
        name: "PE Total OI",
        type: "line",
        data: displayCumulativeOiPoints.map((point) => [Date.parse(point.capturedAt), point.peOi]),
        connectNulls: false,
        showSymbol: displayCumulativeOiPoints.length <= 1,
        symbolSize: 7,
  ...
  }), [displayCumulativeOiPoints, oiDisplayShortLabel, sharedTimeDomain]);
  const cumulativeDifferenceOption = useMemo<EChartsOption>(
    () => scalperV2OiDifferenceOption(displayCumulativeOiPoints, istClock, sharedTimeDomain, dayOpenMs, oiDisplayShortLabel),
    [dayOpenMs, displayCumulativeOiPoints, oiDisplayShortLabel, sharedTimeDomain],
  );
  const cumulativeOiDifferenceOnlyOption = useMemo<EChartsOption>(
    () => scalperV2OiMetricOption(displayCumulativeOiPoints, "oi", istClock, sharedTimeDomain, dayOpenMs, oiDisplayShortLabel),
    [dayOpenMs, displayCumulativeOiPoints, oiDisplayShortLabel, sharedTimeDomain],
  );
  const cumulativeChangeDifferenceOnlyOption = useMemo<EChartsOption>(
    () => scalperV2OiMetricOption(displayCumulativeOiPoints, "change", istClock, sharedTimeDomain, dayOpenMs, oiDisplayShortLabel),
    [dayOpenMs, displayCumulativeOiPoints, oiDisplayShortLabel, sharedTimeDomain],
  );
  ...
  const analyticOptions = useMemo<EChartsOption[]>(() => [
    scalperV2VerticalStrikeOption(strikeRows, ceCurrent, peCurrent, "oi", spot, nearestSpotStrike, oiDisplayShortLabel),
    scalperV2VerticalStrikeOption(strikeRows, ceChanges, peChanges, "change", spot, nearestSpotStrike, oiDisplayShortLabel),
  ...
  ], [ceChanges, ceCurrent, maxPain.points, nearestSpotStrike, oiDisplayShortLabel, payoutNiftyCurrentGuide, peChanges, peCurrent, spot, strikeRows]);
  const structureInput = useMemo(() => {
    const rows = (wanted: "CE" | "PE") => strikeRows.map((strike) => {
      ...
      return {
        oi: history?.oi != null ? scaleScalperV2Oi(history.oi, oiDisplayMultiplier) : profileRows.find((row) => row.side === wanted && row.strike === strike)?.currentOi ?? null,
        changeOi: history?.changeOi != null ? scaleScalperV2Oi(history.changeOi, oiDisplayMultiplier) : profileRows.find((row) => row.side === wanted && row.strike === strike)?.changeOi ?? null,
  ...
  return <section className={css.page} data-testid={isV3 ? "scalper-v3" : "scalper-v2"} data-layout={layout} data-popout={isPopout || undefined} data-right-open={isV3 ? v3RightOpen : undefined} data-bottom-open={isV3 ? v3BottomOpen : undefined} data-maximized-price={v3MaximizedPrice ?? undefined} data-v3-density={isV3 ? v3Density : undefined} data-v3-preset={isV3 ? v3LayoutPreset : undefined} data-v3-live={isV3 ? v3FollowLive : undefined} data-v3-feed={isV3 ? v3FeedStatus : undefined} data-what-changed={isV3 && v3WhatChanged ? true : undefined} style={v3Style} data-potential-ema-state={potentialEmaAvailability.state} data-potential-ema-references={potentialEmaAvailability.state === "READY" ? potentialEmaSignals.length : ""} data-oi-display-mode={effectiveOiDisplayMode} data-oi-lot-size={oiLotContext.lotSize ?? ""}>
    <header className={css.commandBar}>
      ...
      <div className={css.commandGroup}><span>Time</span><label>Session <select value={tradingDay} onChange={(event) => update("day", event.target.value)}>{days.map((day) => <option key={day}>{day}</option>)}</select></label><label>Timeframe <select aria-label="Scalper timeframe" value={interval} onChange={(event) => update("interval", event.target.value)}><option value={1}>1m</option><option value={5}>5m</option><option value={15}>15m</option><option value={60}>1h</option></select></label><button aria-pressed={horizontalView === "day"} onClick={() => { setHorizontalView("day"); setFitRequest((value) => value + 1); }}>Fit day</button></div>
      ...
      <details className={css.commandMenu}><summary>Scale</summary><div><button aria-pressed={verticalView === "session"} onClick={() => { setVerticalView("session"); setYLocked(false); }}>Session Y</button><button aria-pressed={verticalView === "visible"} onClick={() => { setVerticalView("visible"); setYLocked(false); }}>Visible Y</button><button aria-pressed={verticalView === "manual"} onClick={() => { setVerticalView("manual"); setYLocked(false); }}>Manual Y</button><button aria-pressed={yLocked} onClick={() => setYLocked((value) => !value)}>{yLocked ? "Unlock Y" : "Lock Y"}</button><button onClick={() => { setHorizontalView("last30"); setFitRequest((value) => value + 1); }}>Last 30</button><button onClick={() => { setHorizontalView("last60"); setFitRequest((value) => value + 1); }}>Last 60</button></div></details>
      ...
      <div className={css.commandGroup} data-testid="v2-oi-unit-toggle" aria-label="Open interest display unit"><span>OI unit</span><button type="button" aria-pressed={effectiveOiDisplayMode === "contracts"} onClick={() => setOiDisplayMode("contracts")}>Contracts</button><button type="button" aria-pressed={effectiveOiDisplayMode === "underlying_units"} disabled={!oiLotContext.available} title={oiLotContext.available ? `Multiply every OI and change-in-OI display by the exact F&O lot size ${oiLotContext.lotSize}` : oiLotContext.reason ?? "Lot conversion unavailable"} onClick={() => setOiDisplayMode("underlying_units")}>× Lot{oiLotContext.lotSize == null ? "" : ` ${oiLotContext.lotSize}`}</button></div>
    ...
    <div className={css.workspace}>
      ...
      {(!isV3 || v3RightOpen) && <aside className={css.structureCharts} data-testid={`${viewId}-strike-side-charts`} aria-label="Strike open interest comparison charts" onDoubleClick={expandV3Analytic}>
        ...
        <article data-testid="v2-side-oi-chart">
          <header><strong>OI by strike</strong><RefreshStamp at={active.dataUpdatedAt} /><span>{oiDisplayLabel} · CE / PE · PE − CE</span><ChartActions title="OI by strike" onExpand={() => setExpandedChart("oi")} /></header>
        ...
        <article data-testid="v2-side-delta-oi-chart">
          <header><strong>ΔOI by strike</strong>{largestChange && <button type="button" className={css.headerMetric} onClick={() => setV3PinnedStrike(largestChange.strike)} title="Focus the largest absolute change in OI">Largest {largestChange.side} {largestChange.strike.toLocaleString("en-IN")} {signed(largestChange.value)}</button>}<RefreshStamp at={active.dataUpdatedAt} /><span>{oiDisplayLabel} · CE / PE · PE ΔOI − CE ΔOI</span><ChartActions title="Change in OI by strike" onExpand={() => setExpandedChart("delta-oi")} /></header>
        ...
        <article data-testid="v2-side-strike-structure-chart">
          <header><strong>Strike structure</strong><RefreshStamp at={active.dataUpdatedAt} /><span>{oiDisplayLabel} · OI bars · ΔOI lines · premium markers · CE1–5 / PE1–5</span><ChartActions title="Strike structure" onInfo={() => setChartInfo("strike-structure")} onExpand={() => setExpandedChart("strike-structure")} /></header>
      ...
      {railOpen && <aside className={css.rail} aria-label="Scalper V2 option chain and inspector">
        ...
        <table className={css.pairMetrics} aria-label="Selected contracts snapshot metrics"><thead><tr><th>Latest snapshot</th><th className={css.callText}>CE {selectedCeStrike}</th><th className={css.putText}>PE {selectedPeStrike}</th></tr></thead><tbody>
          <tr><th>Open Interest · {oiDisplayShortLabel}</th><td>{compact(callProfile?.currentOi)}</td><td>{compact(putProfile?.currentOi)}</td></tr>
          <tr><th>Change in OI · {oiDisplayShortLabel}</th><td className={signClass(callProfile?.changeOi)}>{signed(callProfile?.changeOi)}</td><td className={signClass(putProfile?.changeOi)}>{signed(putProfile?.changeOi)}</td></tr>
        ...
        <section className={css.structureSummary}><h3>Structure · latest snapshot</h3><div><span>OI PCR <b>{pcr == null ? "—" : pcr.toFixed(2)}</b></span><span>Max Pain <b>{maxPainValue == null ? "—" : maxPainValue.toLocaleString("en-IN")}</b></span><span>Distance <b className={signClass(inspectedUnderlying != null && maxPainValue != null ? inspectedUnderlying - maxPainValue : null)}>{signed(inspectedUnderlying != null && maxPainValue != null ? inspectedUnderlying - maxPainValue : null)} pts</b></span><span>Signal <b>{latestSignal ? `${latestSignal.direction} · ${latestSignal.state === "POTENTIAL_ENTRY_REFERENCE" ? "tentative EMA + volume alignment" : latestSignal.state === "DIRECTIONAL_ENTRY_REFERENCE" ? "OI direction entry reference" : latestSignal.state === "RETROSPECTIVE_ENTRY_REFERENCE" ? "EMA entry reference" : latestSignal.state.replaceAll("_", " ").toLowerCase()}` : "None"}</b></span></div></section>
        <div className={css.leaders}>{displayLeaders.filter((leader) => leader.rank <= 2).map((leader) => <div className={css.leader} key={`${leader.side}-${leader.rank}`}><span className={leader.side === "CE" ? css.callText : css.putText}>{leader.side}{leader.rank}</span><b>{leader.strike.toLocaleString("en-IN")}</b><small>OI {compact(leader.currentOi)} · Δ {signed(leader.changeOi)}</small></div>)}</div>
        ...
        <div className={css.tabs} role="tablist">{(["time", "chain", "profile", "levels", "rules", "measure", "objects", "health"] as const).map((tab) => <button key={tab} role="tab" aria-selected={railTab === tab} onClick={() => setRailTab(tab)}>{tab === "time" ? "Snapshot" : tab === "profile" ? "ΔOI profile" : tab === "rules" ? "Strategy" : tab[0].toUpperCase() + tab.slice(1)}</button>)}</div>
        <div className={css.railBody}>
          ...
          {railTab === "chain" && <table className={css.chain} onMouseLeave={() => setHoveredStrike(null)}><thead><tr><th>CE OI · {oiDisplayShortLabel}</th><th>Select CE</th><th>Strike</th><th>Select PE</th><th>PE OI · {oiDisplayShortLabel}</th></tr></thead><tbody>{strikeRows.map((strike) => { const ce = rankSource.find((row) => side(row) === "CE" && numeric(row.strike) === strike), pe = rankSource.find((row) => side(row) === "PE" && numeric(row.strike) === strike); return <tr key={strike} data-ce-selected={String(strike) === selectedCeStrike || undefined} data-pe-selected={String(strike) === selectedPeStrike || undefined} onMouseEnter={() => setHoveredStrike(strike)}><td>{compact(displayOi(ce?.open_interest))}</td><td><button disabled={points.length > 0 || !ce} aria-pressed={String(strike) === selectedCeStrike} onClick={() => updateLegStrike("CE", String(strike))}>{price(ce?.last_price)}{String(strike) === selectedCeStrike ? " · Selected" : ""}</button></td><th>{strike.toLocaleString("en-IN")}{strike === defaultStrike ? " · ATM" : ""}</th><td><button disabled={points.length > 0 || !pe} aria-pressed={String(strike) === selectedPeStrike} onClick={() => updateLegStrike("PE", String(strike))}>{price(pe?.last_price)}{String(strike) === selectedPeStrike ? " · Selected" : ""}</button></td><td>{compact(displayOi(pe?.open_interest))}</td></tr>; })}</tbody></table>}
          {railTab === "profile" && <section className={css.profileEvidence} aria-label="Accessible strike change in open interest profile">
            <header><strong>Strike-aligned ΔOI</strong><span>{deltaBasisLabel} · {oiDisplayLabel}<br />{profileComparable}/{profileRows.length} comparable legs</span></header>
          ...
          {railTab === "levels" && <><p>Ranked from <strong>{metricLegs.length ? "the retained observed cohort" : "the nearest paired observed window"}</strong>. Values use {oiDisplayLabel.toLowerCase()}; rank identity is unchanged. Off-session leaders remain here and are not promoted.</p>{displayLeaders.map((leader) => <p key={`${leader.side}${leader.rank}`}><b>{leader.side}{leader.rank}</b> {leader.strike.toLocaleString("en-IN")} · OI {leader.currentOi.toLocaleString("en-IN")} · ΔOI {signed(leader.changeOi)}</p>)}</>}
          {railTab === "rules" && <section data-testid="v2-strategy-evidence"><h3>Three-instrument tentative EMA references</h3><p><strong>{SCALPER_V2_THREE_INSTRUMENT_EMA_RULE}</strong></p><p>Completed 5m candles only. CALL requires NIFTY and selected CE to cross/close above EMA9 while selected PE crosses/closes below. PUT is the exact inverse. Each leg must cross on this or the previous candle, with at least two of the prior five closes on its source side. In addition, both the exact CE and exact PE setup-candle volumes must each be at least 95% of their own progressively warmed Volume EMA20. Exact shared timestamps are mandatory.</p><p data-testid="v2-potential-ema-count">{potentialEmaAvailability.state === "READY" ? `Tentative references ${potentialEmaSignals.length}` : potentialEmaAvailability.reasons.join(" · ")} · hollow CE-up yellow and PE-down blue triangles are 60% transparent tentative research markers, not actual or executed trades</p>{potentialEmaSignals.slice(-20).map((signal) => <p key={signal.id}><b>{signal.direction}</b> · tentative reference · {signal.setupTime}<br /><small>{signal.legs.map((leg) => `${leg.instrument} ${leg.targetSide.toLowerCase()} at ${leg.crossTime} (${leg.sourceSideCloses}/5 prior source-side closes${leg.volumeToEmaRatio == null ? "" : ` · volume ${(leg.volumeToEmaRatio * 100).toFixed(1)}% of EMA20`})`).join(" · ")}</small></p>)}<h3>Retained-data evaluation</h3><p data-testid="v2-ema-backtest-scope">Exact selected pair · {potentialEmaEvaluation.sessions} retained session{potentialEmaEvaluation.sessions === 1 ? "" : "s"} · {potentialEmaEvaluation.firstSession ?? "—"} to {potentialEmaEvaluation.lastSession ?? "—"} · {potentialEmaEvaluation.signals} tentative references ({potentialEmaEvaluation.calls} CALL / {potentialEmaEvaluation.puts} PUT)</p><table className={css.snapshotGrid}><thead><tr><th>Forward bars</th><th>Comparable</th><th>Both positive</th><th>Underlying avg</th><th>Selected option avg</th></tr></thead><tbody>{potentialEmaEvaluation.horizons.map((row) => <tr key={row.bars}><th>{row.bars} · {row.bars * potentialEmaEvaluation.intervalMinutes}m</th><td>{row.comparable}</td><td>{row.comparable ? `${row.positiveFollowThrough}/${row.comparable}` : "—"}</td><td>{row.averageUnderlyingPct == null ? "—" : `${signed(row.averageUnderlyingPct)}%`}</td><td>{row.averageSelectedOptionPct == null ? "—" : `${signed(row.averageSelectedOptionPct)}%`}</td></tr>)}</tbody></table><h3>Exact-time return correlation</h3><table className={css.snapshotGrid}><tbody><tr><th>NIFTY ↔ selected CE</th><td>{potentialEmaEvaluation.correlations.underlyingVsCe.value == null ? "—" : potentialEmaEvaluation.correlations.underlyingVsCe.value.toFixed(3)}</td><td>n={potentialEmaEvaluation.correlations.underlyingVsCe.samples}</td></tr><tr><th>NIFTY ↔ selected PE</th><td>{potentialEmaEvaluation.correlations.underlyingVsPe.value == null ? "—" : potentialEmaEvaluation.correlations.underlyingVsPe.value.toFixed(3)}</td><td>n={potentialEmaEvaluation.correlations.underlyingVsPe.samples}</td></tr></tbody></table><p><small>Correlation uses exact aligned close-to-close returns and is descriptive, not causation. Forward evaluation starts at the tentative-reference close, excludes missing timestamps, and reports direction-adjusted NIFTY plus the selected CALL/PUT premium. It does not reconstruct fills, costs, exits or P&amp;L.</small></p><h3>Other retained strategy references</h3><p><strong>{SCALPER_ENTRY_RULE}</strong></p><p>Entry references {signalCounts.RETROSPECTIVE_ENTRY_REFERENCE ?? 0} · waiting {signalCounts.WAIT_NEXT_OPEN ?? 0} · missing {signalCounts.NEXT_BAR_MISSING ?? 0} · failed {signalCounts.NEXT_OPEN_FAILED ?? 0}</p>{signals.slice(-20).map((signal) => <p key={signal.id}><b>{signal.direction}</b> · {signal.state.replaceAll("_", " ")} · {signal.setupTime}</p>)}<p><strong>{SCALPER_DIRECTIONAL_OI_ENTRY_RULE}</strong></p><p>Independent direction/OI references {directionalSignals.filter((signal) => signal.state === "DIRECTIONAL_ENTRY_REFERENCE").length} · option price unavailable {directionalSignals.filter((signal) => signal.state === "OPTION_PRICE_UNAVAILABLE").length}</p>{directionalSignals.slice(-20).map((signal) => <p key={signal.id}><b>{signal.direction}</b> · {signal.state.replaceAll("_", " ")} · {signal.setupTime}<br /><small>OI cross {signed(signal.previousOiDifference)} → {signed(signal.oiDifference)} · ΔOI pressure {signed(signal.changeOiDifference)} vs open {signed(signal.dayOpenChangeOiDifference)} · refs {signal.matchedReferences.join(", ")}</small></p>)}</section>}
    ...
    <section id="scalper-v2-analytics" className={css.analytics} data-testid="v2-analytics-dock">
      <div className={css.structureRibbon} aria-label={`Current option structure summary · ${oiDisplayLabel}`}><span><b>CE OI · {oiDisplayShortLabel}</b>{compact(oiTotals.ceOi)}</span><span><b>PE OI · {oiDisplayShortLabel}</b>{compact(oiTotals.peOi)}</span><span><b>PCR</b>{oiTotals.pcr == null ? "—" : oiTotals.pcr.toFixed(2)}</span><span className={signClass(oiTotals.ceDelta)}><b>CE ΔOI · {oiDisplayShortLabel}</b>{signed(oiTotals.ceDelta)}</span><span className={signClass(oiTotals.peDelta)}><b>PE ΔOI · {oiDisplayShortLabel}</b>{signed(oiTotals.peDelta)}</span><span><b>Max Pain</b>{maxPainValue?.toLocaleString("en-IN") ?? "—"}</span><span><b>CE1</b>{leaders.find((leader) => leader.side === "CE" && leader.rank === 1)?.strike.toLocaleString("en-IN") ?? "—"}</span><span><b>PE1</b>{leaders.find((leader) => leader.side === "PE" && leader.rank === 1)?.strike.toLocaleString("en-IN") ?? "—"}</span><span><b>Signal</b>{latestSignal?.direction ?? "—"}</span></div>
      ...
      {analyticsTab === "oi" && <div className={css.analyticsGrid}><article className={css.analyticCard}><h3>OI by strike · {oiDisplayLabel} <RefreshStamp at={active.dataUpdatedAt} /></h3><Suspense fallback={<p>Loading chart…</p>}><Chart className={css.analyticChart} ariaLabel={`Open interest by strike · ${oiDisplayLabel}`} axisExtentPolicy="native" option={analyticOptions[0]} activeCategoryIndex={activeStrikeIndex} onCategoryHover={(index) => setHoveredStrike(index == null ? null : strikeRows[index] ?? null)} /></Suspense></article><article id="scalper-v2-deltaoi" className={css.analyticCard} data-testid="v2-deltaoi-chart"><h3>Change in OI by strike · {oiDisplayLabel} <RefreshStamp at={active.dataUpdatedAt} /></h3>{deltaState.state === "baseline_unavailable" || deltaState.state === "current_unavailable" ? <div className={css.stateCard} data-testid="v2-deltaoi-state"><strong>{deltaState.state === "baseline_unavailable" ? "Baseline unavailable" : "Current OI unavailable"}</strong><span>{deltaState.comparable}/{deltaState.total} comparable contracts</span></div> : <Suspense fallback={<p>Loading chart…</p>}><Chart className={css.deltaOiChart} ariaLabel={`Change in OI by strike with adaptive signed scale · ${oiDisplayLabel}`} axisExtentPolicy="native" option={analyticOptions[1]} activeCategoryIndex={activeStrikeIndex} onCategoryHover={(index) => setHoveredStrike(index == null ? null : strikeRows[index] ?? null)} /></Suspense>}</article></div>}
      ...
      {analyticsTab === "total" && <article className={css.dockPanel} data-testid="v2-cumulative-oi-time"><h3>Total OI, differences and PCR vs Time · {oiDisplayLabel} <RefreshStamp at={active.dataUpdatedAt} /></h3><div className={css.overviewKpis}><article><small>CE Total OI · {oiDisplayShortLabel}</small><b>{compact(cumulativeLatest?.ceOi)}</b></article><article><small>PE Total OI · {oiDisplayShortLabel}</small><b>{compact(cumulativeLatest?.peOi)}</b></article><article><small>OI PCR</small><b>{cumulativeLatest?.pcr?.toFixed(3) ?? "—"}</b></article><article><small>Snapshot coverage</small><b>{cumulativeOiComplete}/{cumulativeOiPoints.length} OI · {cumulativeChangeComplete}/{cumulativeOiPoints.length} ΔOI</b><small>{cumulativeStrikeCounts.length ? `${cumulativeStrikeCounts.join("–")} strikes` : "Unavailable"}</small></article></div>{cumulativeOiPoints.length ? <div className={css.oiTimeGrid}><section className={css.oiTimeWide}><h4>CE and PE total OI · {oiDisplayLabel} <RefreshStamp at={active.dataUpdatedAt} /></h4><Suspense fallback={<p>Loading chart…</p>}><Chart className={css.cumulativeOiChart} ariaLabel={`Total CE and PE open interest over time · ${oiDisplayLabel}`} axisExtentPolicy="native" option={cumulativeOiOption} activeTimeMs={inspectionTime == null ? null : inspectionTime * 1000} onTimeHover={(value) => handleOiTimeHover(value, "total-oi")} /></Suspense></section><section data-testid="v2-oi-differences-time"><h4>Put minus call OI and reported change in OI · {oiDisplayLabel} <RefreshStamp at={active.dataUpdatedAt} /></h4><Suspense fallback={<p>Loading difference chart…</p>}><Chart className={css.oiDifferenceChart} ariaLabel={`Put minus call total OI and put minus call reported change in OI over time · ${oiDisplayLabel}`} axisExtentPolicy="native" option={cumulativeDifferenceOption} activeTimeMs={inspectionTime == null ? null : inspectionTime * 1000} onTimeHover={(value) => handleOiTimeHover(value, "oi-differences")} /></Suspense></section><section data-testid="v2-pcr-time"><h4>OI PCR over time · PE OI / CE OI <RefreshStamp at={active.dataUpdatedAt} /></h4><Suspense fallback={<p>Loading PCR chart…</p>}><Chart className={css.oiDifferenceChart} ariaLabel="Open interest PCR over time" axisExtentPolicy="native" option={cumulativePcrOption} activeTimeMs={inspectionTime == null ? null : inspectionTime * 1000} onTimeHover={(value) => handleOiTimeHover(value, "pcr-time")} /></Suspense></section></div> : <div className={css.stateCard}><strong>OI history unavailable</strong></div>}</article>}
