...
import { scalperV2CompletedCandleSignature, scalperV2RefreshClock, shouldFollowScalperV2TradingDay, shouldRefitScalperV2Day } from "../lib/scalperV2LiveSession";
import {
...
export function TradingAnalyticsScalperV2({ symbol, label, asOf, expiry, strikes, spot, legs, metricLegs = [], state, errors = [], referenceLevels }: {
  ...
  useEffect(() => {
    ...
    if (shouldRefitScalperV2Day({ historical: Boolean(replayAsOf), horizontalView, previousSignature, nextSignature: completedCandleSignature })) setFitRequest((value) => value + 1);
  }, [completedCandleSignature, horizontalView, replayAsOf]);
  ...
  return <section className={css.page} data-testid="scalper-v2" data-popout={isPopout || undefined}>
    ...
    <div className={css.workspace}>
      ...
      <aside className={css.structureCharts} data-testid="v2-strike-side-charts" aria-label="Strike open interest comparison charts">
        <article>
          <header><strong>OI by strike</strong><RefreshStamp at={active.dataUpdatedAt} /><span>CE / PE · PE − CE</span></header>
        ...
        <article data-testid="v2-side-delta-oi-chart">
          <header><strong>ΔOI by strike</strong><RefreshStamp at={active.dataUpdatedAt} /><span>CE / PE · PE ΔOI − CE ΔOI</span></header>
        ...
        <article data-testid="v2-side-strike-structure-chart">
          <header><strong>Strike structure</strong><RefreshStamp at={active.dataUpdatedAt} /><span>OI bars · ΔOI lines · premium markers · CE1–5 / PE1–5</span></header>
        ...
        <article data-testid="v2-side-positioning-heatmap">
          <header><strong>Strike × time positioning</strong><RefreshStamp at={optionPriceHistory.dataUpdatedAt} /><span>{interval === 15 ? "15m" : "5m"} · ΔOI share + premium + volume + depth</span></header>
