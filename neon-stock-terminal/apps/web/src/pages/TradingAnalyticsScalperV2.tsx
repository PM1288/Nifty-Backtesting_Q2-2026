...
export function TradingAnalyticsScalperV2({ symbol, label, asOf, expiry, strikes, spot, legs, metricLegs = [], state, errors = [], referenceLevels }: {
  ...
  return <section className={css.page} data-testid="scalper-v2" data-popout={isPopout || undefined}>
    ...
    <header className={css.commandBar}>
      ...
      {!isPopout && <button type="button" data-testid="v2-popout" onClick={openPopout}>Pop out</button>}
    ...
    <div className={css.workspace}>
      ...
      <aside className={css.structureCharts} data-testid="v2-strike-side-charts" aria-label="Strike open interest comparison charts">
        <article>
          ...
          <Suspense fallback={<p>Loading OI chart…</p>}><Chart className={css.structureChart} ariaLabel="Open interest by strike with put minus call difference" axisExtentPolicy="native" option={analyticOptions[0]} activeCategoryIndex={hoveredStrikeIndex} onCategoryHover={(index) => setHoveredStrike(index == null ? null : strikeRows[index] ?? null)} /></Suspense>
        </article>
