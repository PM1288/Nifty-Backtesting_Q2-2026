...
def attach_context(df,stock,nifty,glob):
 ...
 return x
def select_trials(matrix,ids,max_trials):
...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--database-url',default=os.getenv('DATABASE_URL') or os.getenv('TRADING_DATABASE_URL')); p.add_argument('--symbol',default='RELIANCE'); p.add_argument('--start',type=date.fromisoformat,default=date(2024,1,1)); p.add_argument('--end',type=date.fromisoformat,default=date(2025,12,31)); p.add_argument('--trial-id',action='append'); p.add_argument('--max-trials',type=int,default=3); p.add_argument('--minute-csv-dir',type=Path,default=shared.DEFAULT_MINUTE_CSV_DIR); p.add_argument('--output-root',type=Path,default=PROJECT/'outputs/oiis_component_doe_v1'); a=p.parse_args()
 if not a.database_url: raise SystemExit('DATABASE_URL required')
 ensure_delivery()
 matrix=pd.read_csv(MATRIX); selected=select_trials(matrix,a.trial_id,a.max_trials)
 if selected.empty:raise SystemExit('No DOE trials selected')
 run_id=str(uuid.uuid4()); out=a.output_root/run_id; out.mkdir(parents=True); experiment=json.loads(EXPERIMENT.read_text()); all_components=[]; all_trades=[]; all_targets=[]; all_adverse=[]; summaries=[]
 with psycopg.connect(a.database_url,row_factory=dict_row) as conn:
  prices,regimes=shared.load_source(conn,a.start,a.end,a.symbol); features=shared.derive_features(prices,regimes); stock,nifty,glob=load_context(conn,a.start-timedelta(days=7),a.end,a.symbol)
  ...
  for row in selected.itertuples(index=False):
   ...
   all_components.append(components); all_trades.append(trade_frame); all_targets.extend(target_rows(row.run_id,trades,'TARGET')); all_adverse.extend(target_rows(row.run_id,trades,'ADVERSE')); summaries.append(trial_summary(row,decisions,trades)); print(row.run_id,'decisions',len(decisions),'trades',len(trades),flush=True)
  comp=pd.concat(all_components,ignore_index=True); trades=pd.concat(all_trades,ignore_index=True) if all_trades else pd.DataFrame(); summary=pd.DataFrame(summaries); baseline=summary.iloc[0]; effects=summary.copy()
  ...
  comp.to_csv(out/'component_event_scores.csv',index=False); trades.to_csv(out/'trades.csv',index=False); summary.to_csv(out/'trial_summary.csv',index=False); effects.to_csv(out/'factor_effects_vs_baseline.csv',index=False); pd.DataFrame(all_targets).to_csv(out/'target_events.csv',index=False); pd.DataFrame(all_adverse).to_csv(out/'adverse_events.csv',index=False); regime_perf.to_csv(out/'regime_performance.csv',index=False)
  ...
  conn.execute(SCHEMA.read_text()); conn.execute('INSERT INTO strategy_eval.oiis_doe_run VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s::jsonb,now())',(run_id,experiment['experiment_id'],a.symbol,a.start,a.end,len(summary),len(comp),len(trades),'SMOKE_SUCCEEDED',str(out.resolve()),json.dumps({'matrix_sha256':digest(MATRIX),'experiment_sha256':digest(EXPERIMENT)})))
  with conn.cursor() as cur:
   ...
   cur.executemany('INSERT INTO strategy_eval.oiis_doe_component_event VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',component_rows)
  conn.commit()
 (out/'README.md').write_text(f'# OIIS component DOE smoke\n\nRun `{run_id}` evaluated {len(summary)} governed trials for `{a.symbol}`. `trades.csv` contains all trades in one file. `component_event_scores.csv` contains actual O/X component values, weights and contributions. Regimes are point-in-time PostgreSQL joins. OIIS remains entry-only; shared exit outcomes are evaluation scenarios.\n'); print(json.dumps({'doe_run_id':run_id,'trials':len(summary),'component_events':len(comp),'trades':len(trades),'output':str(out)},indent=2))
