...
function computeWillrSeries(highs: number[], lows: number[], closes: number[], period = 14): Array<number | null> {
  ...
  for (let i = period - 1; i < closes.length; i += 1) {
    ...
    const willr = ((highestHigh - closes[i]!) / range) * -100;
    out[i] = clamp(willr, -100, 0);
  }
...
function emptyPayload(asOf: string, indices: SurfacePayload["indices"]): SurfacePayload {
  ...
  return {
    ...
    colorScale: { min: -100, max: 0, yellowPivot: -50, greenPivot: -20 },
    stats: { min: -50, max: -50, avg: -50, niftyWillr: indices.nifty50.willr ?? null, universeAvgWillr: -50 },
    indices
  };
}

export async function buildWillSurfacePayload(prisma: PrismaClient): Promise<SurfacePayload> {
  ...
  const barsBySymbol = new Map<string, Array<{ ts: string; high: number; low: number; close: number }>>();
  ...
  for (const row of rawRows) {
    ...
    if (!ts || high == null || low == null || close == null) continue;
    if (!barsBySymbol.has(row.symbol)) barsBySymbol.set(row.symbol, []);
    barsBySymbol.get(row.symbol)!.push({ ts, high, low, close });
  }
  ...
  for (const item of universe) {
    ...
    const willrSeries = smoothSeries(computeWillrSeries(highs, lows, closes, 14), 0.22);
    const willrByTs = new Map<string, number>();

    for (let i = 0; i < bars.length; i += 1) {
      const willr = willrSeries[i];
      if (willr == null || !Number.isFinite(willr)) continue;
      willrByTs.set(bars[i]!.ts, clamp(willr, -100, 0));
    }

    const rowValues: number[] = [];
    let carry = Number.isFinite(quote?.willr) ? Number(quote?.willr) : -50;
    for (const ts of timelineIso) {
      const next = willrByTs.get(ts);
      if (next != null) {
        carry = next;
      }
    ...
    const latestWillr = rowValues[rowValues.length - 1] ?? carry;
    const row: SurfaceRow = {
      ...
      latestWillr: Number(latestWillr.toFixed(2))
    };
  ...
  const sortedSectors = [...sectorMap.entries()]
    ...
    .map(([sector, rows]) => ({
      sector,
      rows: [...rows].sort((a, b) => {
        if (b.latestWillr !== a.latestWillr) return b.latestWillr - a.latestWillr;
  ...
  for (const entry of sortedSectors) {
    ...
    const avgWillr = rows.length ? rows.reduce((sum, row) => sum + row.latestWillr, 0) / rows.length : -50;
    sectors.push({
      sector: entry.sector,
      startIndex: cursor,
      count: rows.length,
      avgWillr: Number(avgWillr.toFixed(2)),
      rows
    });
    cursor += rows.length;
  }
  ...
  return {
    ...
    stats: {
      ...
      niftyWillr: indices.nifty50.willr ?? null,
      universeAvgWillr: Number(
        (orderedRows.reduce((sum, row) => sum + row.latestWillr, 0) / Math.max(1, orderedRows.length)).toFixed(2)
      )
    },
    indices
  };
  ...
  app.get("/v1/will-surface", async (req, res) =>
    serveSnapshotRoute(req, res, prisma, {
      ...
      build: async (db) => {
        ...
        const payload = await buildWillSurfacePayload(db);
        cacheEntry = {
          key: cacheKey,
