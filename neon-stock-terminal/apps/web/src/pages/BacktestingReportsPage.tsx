...
export function BacktestingReportsPage() {
  ...
  return <main className={styles.page} data-testid="backtesting-reports">
    ...
    {query.isLoading ? <section className={styles.state}>Loading report catalogue…</section> : query.isError || !report ? <section className={styles.state} role="alert">The generated report is not currently mounted. No placeholder results are shown.</section> : <>
      ...
      <section className={styles.notes}><h2>Report contents</h2><p>The first PDF page combines the exact Bull/Bear conditions, limitations, colour legend and aggregate summary. Every stock then has its own performance summary and Daily/Weekly/Monthly chart page, followed immediately by one or more paginated trade-evidence tables.</p><p>Each stock table includes signal date, direction, both entry bases, causal outcomes, all six Month/Week/Day equations, M−1/M−2/M−3 open-close values and individual/group pass states. Daily triangles retain exact dates; Weekly and Monthly triangles aggregate those events into the containing candle. The Daily chart includes month/week boundaries and distinct open and retrospective final-close levels. The CSV remains the complete machine-readable ledger. Historical 1H/15m confirmation is not claimed because retained minute data does not cover the full evaluation year.</p></section>
    </>}
  </main>;
}
