...
REQUIRED_TABLES = (
    "catalog.option_contract_observation",
    "research.option_trade_result",
)
        ...
        with psycopg.connect(dsn, autocommit=True) as conn, conn.cursor() as cur:
            ...
            if result["checks"].get("table:catalog.option_contract_observation"):
                cur.execute(
                    ...
                    FROM catalog.option_contract_observation
                    WHERE upper(underlying_symbol) = upper(%s)
                    """,
                    (args.underlying,),
                )
                count, first_seen, last_seen, source_count = cur.fetchone()
                result["checks"]["contract_observations"] = int(count)
                result["checks"]["contract_first_available_at"] = first_seen.isoformat() if first_seen else None
                result["checks"]["contract_last_available_at"] = last_seen.isoformat() if last_seen else None
                result["checks"]["contract_source_count"] = int(source_count)
                if count == 0:
                    result["blockers"].append("no point-in-time NIFTY option contract observations")

            cur.execute(
                """
                SELECT to_regclass('catalog.option_quote_observation'),
                       to_regclass('market.option_quote_observation')
                """
            )
